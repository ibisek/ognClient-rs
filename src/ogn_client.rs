...
use crate::configuration::{SERVER_ADDR, AIRCRAFT_REGEX};
...
// impl<'a> Observer<String> for &'a mut OgnClient {
// impl Observer<String> for &mut OgnClient {
impl Observer<String> for OgnClient {
    fn notify(&self, line: &String) {
        let beaconOpt = self.parse_beacon_line(&line);
        if beaconOpt.is_some() {
            let beacon = beaconOpt.unwrap();
...
impl OgnClient {
    ...
    pub fn connect(&mut self) {
        self.server.connect();
        self.server.add_line_listener(&self);    //Arc::new(self)
    }
    ...
    fn parse_beacon_line(&self, line: &str) -> Option<AircraftBeacon> {
        lazy_static! {
            ...
            static ref SUPPORTED_BEACONS: Vec<String> = vec!["OGN".to_string(), "FLR".to_string(), "ICA".to_string()];
        }
        ...
        let ts = OgnClient::rx_time_to_utc_ts(rx_time);   // convert rx_time to UTC ts
